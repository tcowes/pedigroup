...
def format_group_orders_with_date(orders: list[GroupOrder]) -> str:
    ...
    return "\n\n".join([f"El {order.created_at.strftime("%d/%m/%Y")} se pidió:\n{format_order(order.orders.all())}" for order in orders])